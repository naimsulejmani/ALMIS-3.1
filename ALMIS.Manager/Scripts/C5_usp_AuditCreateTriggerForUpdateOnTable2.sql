	...
	BEGIN
	    ...
	    '''
	               AND sys.fn_IsBitSetInBitmask(
	                       CAST(COLUMNS_UPDATED() AS VARBINARY(254)),
	                       COLUMNPROPERTY(
		...
		begin
			SELECT @fieldID = ac.ColumnID
	        FROM   Almis.AuditColumns ac
	        WHERE  ac.TableID IN (SELECT at.TableID
	                              FROM   Almis.AuditTables at
	                              WHERE  at.TableName = ''' + @tableName +
	    ''' and at.TableSchema ='''+@tableSchema+''')
	               AND ac.ColumnName = @fieldName AND ac.AuditColumn=1
		...
		SELECT @CaptureCount=COUNT(*)
		FROM Almis.AuditLogDetail ald
		WHERE ald.AuditLogID=@AuditID

		IF(@CaptureCount>0)
		EXEC
			Almis.usp_AuditLogTrailer
			@AuditID,@TableID,
			@RowsAffected,
			''U'',
			@AuditedUser
