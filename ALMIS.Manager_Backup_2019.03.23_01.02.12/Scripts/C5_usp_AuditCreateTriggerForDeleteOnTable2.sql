CREATE PROCEDURE [Almis].[usp_AuditCreateTriggerForDeleteOnTable]
	@tableSchema varchar(128),
	@tableName VARCHAR(128)
AS
BEGIN
	DECLARE @sql VARCHAR(MAX)
	SET @sql = 'CREATE TRIGGER [Almis_Tr_Audit_' + @tableName + '_Del]
	...
	BEGIN
		...
		BEGIN
			SET @sql =
			''INSERT INTO Almis.AuditLogDetail (AuditLogID,RowKey,ColumnID,[STATUS],OldValue)''
		...
		CLOSE kolona
		DEALLOCATE kolona
		EXEC Almis.usp_AuditLogTrailer @AuditID,@TableID,@RowsAffected,''D'',@AuditedUser
	END
	END'
